...
4. Or make use of `npx npm-check-updates <package1> <package2> <packageN>` command to perform batch categories update

   In this case you'd want to specify all the libraries with green updates first

5. Run `npm install` to update the deps

6. Run the project, check if everything is okay

7. If you are done, commit the changes and make a PR, if not, repeat steps 2-5 but with cyan and red updates
